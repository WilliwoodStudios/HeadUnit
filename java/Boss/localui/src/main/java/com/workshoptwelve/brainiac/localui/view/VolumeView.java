...
public class VolumeView extends View {
    ...
    private static final Log log = Log.getLogger(VolumeView.class);
    ...
    private void checkThemeColor() {
        try {
            JSONObject theme = SystemThemeExtension.getSystemTheme();
            if (theme != null && theme.has("backgroundImageColor")) {
                String colorAsString = theme.getString("backgroundImageColor");
                colorAsString = colorAsString.replaceAll("#", "");
                mLine.setColor(Integer.parseInt(colorAsString, 16));
                mLine.setAlpha(255);
            } else {
                log.e("No theme...");
            }
        } catch (JSONException je) {
            log.e("Not working", je);
        } catch (RuntimeException re) {
            log.e("Not working", re);
        }
    }
